...
double OneGoodnessCalculated::tof_no_retro() const{
  double tof = hittime - vertex.t() - offset - distance_position_pmt()/v_light_in_water;
  return tof;
}

double OneGoodnessCalculated::tof(double distance) const
{
  double tof = hittime - vertex.t() - offset - distance/v_light_in_water;
  return tof;
}


double OneGoodnessCalculated::tof_on_retro() const{
  double tof = hittime - vertex.t() - offset - distance_fly_retro()/v_light_in_water;
  return tof;
}
