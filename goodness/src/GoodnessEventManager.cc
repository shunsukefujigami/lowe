...
void GoodnessEventManager::SetParameters()
{
  ...
  double sigma = GoodnessManager::GetGoodnessManager()->GetGoodnessParameters()->Getsigma();
  ...
  for(size_t k = 0;k != ncherenkovdigihits;k++)
    {
      ...
      hittimeminusoffset_minus_vdistancenoretrocrossrv_lightinwater[k] = hittimeminusoffset[k] - d_position_pmt*rv_light_in_water;
      hittimeminusoffset_minus_vdistanceonretrocrossrv_lightinwater[k] = hittimeminusoffset[k] - d_fly_retro*rv_light_in_water;;
    }
}
